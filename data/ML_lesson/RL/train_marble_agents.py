...
class CustomFeatureExtractor(BaseFeaturesExtractor):
    """
    自定义特征提取器，用于处理双智能体环境的观测
    """
    def __init__(self, observation_space: gym.spaces.Dict, features_dim: int = 128):
        super().__init__(observation_space, features_dim)
        
        # 获取观测空间的维度
        obs_dim = observation_space.shape[0]  # 对于单个智能体的观测
...
class DualAgentPPO:
    ...
    def evaluate(self, episodes=10):
        ...
        for ep in range(episodes):
            ...
            while not done:
                # 获取两个智能体的动作
                attacker_action, _ = self.attacker_model.predict(
                    obs['attacker'], deterministic=True
                )
                defender_action, _ = self.defender_model.predict(
                    obs['defender'], deterministic=True
                )
    ...
    def load_models(self, attacker_path="marble_attacker_model", defender_path="marble_defender_model"):
        ...
        if os.path.exists(attacker_path + ".zip"):
            self.attacker_model = PPO.load(attacker_path)
        if os.path.exists(defender_path + ".zip"):
            self.defender_model = PPO.load(defender_path)
...
def main():
    ...
    """
    print("创建弹珠对抗环境...")
    # 由于我们的环境是自定义的字典空间环境，需要特别处理
    # 这里我们创建一个简化版本用于演示
    env = MarbleCombatEnv(render_mode=None)  # 使用无渲染模式进行训练
    
    print("初始化双智能体PPO训练器...")
    dual_agent_trainer = DualAgentPPO(env)
    
    print("开始交替训练...")
    dual_agent_trainer.alternating_train(total_timesteps=20000, cycles=5)
    
    print("\n训练完成！评估最终性能...")
    avg_attacker_reward, avg_defender_reward, avg_score_a, avg_score_b = dual_agent_trainer.evaluate(20)
    ...
    for step in range(500):
        # 使用训练好的模型进行预测
        attacker_action, _ = dual_agent_trainer.attacker_model.predict(
            obs['attacker'], deterministic=True
        )
        defender_action, _ = dual_agent_trainer.defender_model.predict(
            obs['defender'], deterministic=True
        )
