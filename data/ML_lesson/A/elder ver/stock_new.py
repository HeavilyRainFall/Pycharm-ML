...
class StockPredictor:
    ...
    def add_technical_indicators(self, data):
        ...
        try:
            ...
            # 相对强弱指标 (RSI)
            data['RSI'] = ta.momentum.RSIIndicator(data['收盘'], window=14).rsi()

            # MACD
            macd = ta.trend.MACD(data['收盘'])
            data['MACD'] = macd.macd()
            data['MACD_Signal'] = macd.macd_signal()
            data['MACD_Histogram'] = macd.macd_diff()

            # 布林带
            bollinger = ta.volatility.BollingerBands(data['收盘'])
            data['BB_Upper'] = bollinger.bollinger_hband()
            data['BB_Lower'] = bollinger.bollinger_lband()
            data['BB_Middle'] = bollinger.bollinger_mavg()

            # 随机指标
            data['Stoch_K'] = ta.momentum.StochasticOscillator(data['最高'], data['最低'], data['收盘']).stoch()
            data['Stoch_D'] = ta.momentum.StochasticOscillator(data['最高'], data['最低'], data['收盘']).stoch_signal()

            # 成交量指标
            data['Volume_MA'] = data['成交量'].rolling(window=20).mean()
    ...
    def backtest_strategy(self, data, predictions, strategy_type="simple",
        ...
        for i in range(1, len(predictions)):
            ...
            # 获取技术指标
            rsi = data_with_indicators['RSI'].iloc[
                self.time_window + i] if 'RSI' in data_with_indicators.columns else 50
            ma_short_val = data_with_indicators[f'MA{ma_short}'].iloc[
                self.time_window + i] if f'MA{ma_short}' in data_with_indicators.columns else current_price
            ma_long_val = data_with_indicators[f'MA{ma_long}'].iloc[
                self.time_window + i] if f'MA{ma_long}' in data_with_indicators.columns else current_price

            # 预测明日涨跌
            predicted_tomorrow = predictions[i]
            predicted_today = predictions[i - 1]
            predicted_change = (predicted_tomorrow - predicted_today) / predicted_today
            ...
            if strategy_type == "simple":
                ...
                # 动量策略：基于价格动量
                price_momentum = (current_price - data['收盘'].iloc[self.time_window + i - 5]) / data['收盘'].iloc[
                    self.time_window + i - 5]
        ...
        if position == 1:  # 如果最后还持有股票，按最后价格卖出
            ...
            profit = (actual_prices[-1] - entry_price) / entry_price
        ...
        sharpe_ratio = np.mean(returns) / np.std(returns) * np.sqrt(252) if len(returns) > 0 and np.std(
            returns) > 0 else 0

        # 计算年化收益率
        total_days = len(portfolio_values)
        annual_return = (final_value / initial_capital) ** (365 / total_days) - 1 if total_days > 0 else 0
        ...
        return {
            ...
            'dates': actual_dates[1:len(predictions)],
