...
//	obj = (wave_object)malloc(sizeof(struct wave_set) + sizeof(float)* 4 * retval);
	obj = (wave_object)mymalloc(sramx,(sizeof(struct wave_set)));
	if(obj == NULL){
			printf("\n mymalloc error1. Exiting \n");
			return obj;
	}
	obj->filtlength = retval;
...
//		obj = (wt_object)malloc(sizeof(struct wt_set) + sizeof(float)* (siglength + 2 * J * (size + 1)));
		obj = (wt_object)mymalloc(sramx,(sizeof(struct wt_set)));
		if(obj == NULL){
				printf("\n mymalloc error2. Exiting \n");
				return obj;
		}
...
//		obj = (wt_object)malloc(sizeof(struct wt_set) + sizeof(float)* (siglength + 2 * J * (size + 1)));
    ...
    memset(obj->thresholds, 0, sizeof(float) * 10);
	return obj;
}
...
void dwt(wt_object wt, float *inp) {
	...
	wt->outlength = 0;
	wt->zpad = 0;
	orig = (float*)mymalloc(sramx,(sizeof(float)* temp_len));
	if(orig == NULL){
			printf("\n mymalloc error4. Exiting \n");
			return;
	}
	orig2 = (float*)mymalloc(sramx,(sizeof(float)* temp_len));
	if(orig == NULL){
			printf("\n mymalloc error5. Exiting \n");
			return;
	}
	/*
	if ((temp_len % 2) == 0) {
...
void calcThreshold(float *coeffs, float thr)
{
//    if (fabs(*coeffs) <= thr)
//        *coeffs = 0.0;
    if (fabs(*coeffs) <= thr) {
...
static void idwt_sym(wt_object wt, int level, float *cA, int len_cA, float *cD, int len_cD, float *X) {
...
//cancel
//                if(level < 10)
//                    calcThreshold(&cD[i - l], wt->thresholds[level]); //浣庨€氭护娉㈤槇鍊�
				X[m] += wt->wave->lpr[t] * cA[i - l] + wt->wave->hpr[t] * cD[i - l];
				X[n] += wt->wave->lpr[t + 1] * cA[i - l] + wt->wave->hpr[t + 1] * cD[i - l];
			}
		}
...
//	out = (float*)malloc(sizeof(float)* (wt->siglength + 1));
	out = (float*)mymalloc(sramx,(sizeof(float)* (wt->siglength + 1)));
	if(out == NULL){
			printf("\n mymalloc error5. Exiting \n");
			return;
	}
...
//		X_lp = (float*)malloc(sizeof(float)* (N + 2 * lf - 1));
		X_lp = (float*)mymalloc(sramx,(sizeof(float)* (N + 2 * lf - 1)));
		if(X_lp == NULL){
				printf("\n mymalloc error6. Exiting \n");
				return;
		}
		iter = app_len;
...
//		X_lp = (float*)malloc(sizeof(float)* (N + 2 * lf - 1));
		...
		for (i = 0; i < J; ++i) {
			...
			//idwt1(wt, temp, cA_up, out, det_len, wt->output + iter, det_len, X_lp, X_hp, out);
            idwt_sym(wt, i, out, det_len, wt->output + iter, det_len, X_lp);
...
int setThreshold(wt_object wt, const int level, const float threshold) {
    if(level >= 10)
        return -1;
    wt->thresholds[level] = threshold;
    return 0;
}
